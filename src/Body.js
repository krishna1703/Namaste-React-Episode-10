...
const Body = () => {
  ...
  const fetchData = async () => {
    ...
    SetlistofRes(Data?.data?.cards[2]?.data?.data?.cards);

    SetFilteredlistofRes(Data?.data?.cards[2]?.data?.data?.cards);
  };
       ...
       shadow-lg text-center gap-5 bg-gray-50">
        ...
        <div className="flex mx-10 gap-10">
          <button
            ...
            onClick={() => {
              const lists = listofRes.filter((res) => res.data.avgRating < 3);
          ...
          <button
            className=" p-2  bg-green-500  rounded-md text-white"
            onClick={() => {
              const lists = listofRes.filter((res) => res.data.avgRating > 4.0);
      ...
      <div className="flex flex-wrap m-2 justify-center ">
        {searchResult ? (
          FilteredlistofRes.map((res) => {
            return (
              <Link to={"/restaurents/" + res.data.id} key={res.data.id}>
